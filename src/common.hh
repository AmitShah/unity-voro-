...
/*porting voro++ to unity
 
 -Xcode create bundle
 -add libvoro reference
 -add libvoro search path to build settings
 -define wrapper class
 
 build bundle: Location of Release in: /Users/amitshah/Library/Developer/Xcode/DerivedData/voronoi_gen-fnuoyylwpvevmjbrwysrrhsnicgo/Build/Products/Debug/*.bundle
 
 
 In Unity, Put the bundle in Assets/Plugin/ directory
 
 In C#:
 
 [DllImport("voronoi_gen", EntryPoint="ReturnDouble")]
	private static extern double ReturnDouble ();
 
 
 then call as you would
 
 
 -unmarshalling an array of ints from C++ to C#
 
 http://stackoverflow.com/questions/3904224/declaring-a-pointer-to-multidimensional-array-and-allocating-the-array
 
 we could use the out syntax…
 
 
 Enable Unsafe Mode for Debugging in ModoDevelop
